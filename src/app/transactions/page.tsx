...
export default function TransactionsPage() {
  ...
  return <><Header /><main className="mx-auto max-w-7xl px-6 py-10">
    ...
    <div className="panel mt-7 grid gap-3 md:grid-cols-[1fr_160px_160px]">
      ...
      <label className="sr-only" htmlFor="system">System evidence</label><select id="system" value={system} onChange={(event) => setSystem(event.target.value)}><option value="all">All systems</option>{["Invoice", "AR", "GL", "Revenue", "Tax", "Receipt"].map((item) => <option key={item}>{item}</option>)}</select>
    </div>
