...
export function reconcileTransaction(transaction: Transaction): Finding[] {
  ...
  if (ar && !receipt) findings.push({ id: "missing-receipt", severity: "warning", title: "AR balance has no receipt event", evidence: `AR reference ${ar.reference} remains unsupported by a Receipt event for ${transaction.currency} ${ar.amount.toLocaleString()}.`, recommendation: "Confirm whether payment was received and post or import the receipt evidence." });
